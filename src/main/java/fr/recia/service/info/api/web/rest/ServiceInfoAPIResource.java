...
public class ServiceInfoAPIResource {
	...
	@RequestMapping(value = "/" + ApiEndpoints.SERVICE_INFO + "/{fname}", method = RequestMethod.GET, produces = {MediaType.APPLICATION_JSON_VALUE})
	public ResponseEntity<ServiceInfoDto> getServiceInfo(@PathVariable String fname, HttpServletResponse response) {
		...
		try {
			...
			log.error(String.format(ApiEndpoints.SERVICE_INFO + " - Service '%s' non trouvé", fname));
			return new ResponseEntity<>(HttpStatus.NOT_FOUND);
		} catch (Exception e) {
			log.error(ApiEndpoints.SERVICE_INFO + " - ", e);
		}
